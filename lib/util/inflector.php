...
#
   ...
   abstract class Inflector
   {
      ...
      static function irregular($singular, $plural=null) {
         ...
         } elseif (strtoupper($singular[0]) == strtoupper($plural[0])) {
            self::plural(
               '#('.$singular[0].')'.substr($singular, 1).'$#i',
               '\1'.substr($plural, 1)
            );
            self::singular(
               '#('.$plural[0].')'.substr($plural, 1).'$#i',
               '\1'.substr($singular, 1)
            );
         } else {
            self::plural(
               '#'.strtoupper($singular[0]).'(?i)'.substr($singular, 1).'$#',
               ucfirst($plural)
            );
            self::plural(
               '#'.strtolower($singular[0]).'(?i)'.substr($singular, 1).'$#',
               strtolower($plural[0]).substr($plural, 1)
            );
            self::singular(
               '#'.strtoupper($plural[0]).'(?i)'.substr($plural, 1).'$#',
               ucfirst($singular)
            );
            self::singular(
               '#'.strtolower($plural[0]).'(?i)'.substr($plural, 1).'$#',
               strtolower($singular[0]).substr($singular, 1)
            );
         }
      }
      ...
      static function pluralize($word, $singular=null, $plural=null) {
         if ($singular and $plural) {
            return $word == 1 ? "$word $singular" : "$word $plural";
         } elseif ($singular) {
            return self::pluralize($word, $singular, self::pluralize($singular));
         } elseif (blank($word) or in_array(strtolower($word), self::$_uncountables)) {
            return $word;
         } else {
      ...
      static function singularize($word) {
         if (blank($word) or in_array(strtolower($word), self::$_uncountables)) {
            return $word;
         } else {
      ...
      function underscore($word) {
         return strtolower(preg_replace('/\s+/', '_',
      ...
      function parameterize($string, $sep='-') {
         ...
         $string = preg_replace("/($re_sep)+/i", $sep, $string);
